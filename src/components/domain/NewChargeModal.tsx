...
export function NewChargeModal({ onClose, onCreated, defaultMonth }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!res.ok) { setError('Erro ao criar cobrança'); setSaving(false); return }
    onCreated()
  }
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#2a2a2a]">
          <h2 className="text-sm font-semibold">Nova cobrança</h2>
