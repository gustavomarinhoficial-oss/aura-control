export const TOOL_META: Record<string, { kind: 'consulting' | 'executing'; label: string }> = {
  ...
  list_meetings: { kind: 'consulting', label: 'Consultando reuniões' },
  ...
  create_meeting: { kind: 'executing', label: 'Agendando reunião' },
  update_meeting: { kind: 'executing', label: 'Atualizando reunião' },
